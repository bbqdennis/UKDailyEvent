...
- `css/styles.css` 包含版面與配色設定，並定義活動卡片、可點擊地點、單行顯示的活動連結、聯絡我們區塊與複製按鈕樣式。
- `js/main.js` 擔負資料載入、轉換與 DOM 產生，使用 JSON 原始連結文字並讓地點欄位可跳轉到 Google Maps 搜尋，在連結、價錢、簡介等欄位為 N/A 時自動省略顯示，並提供複製卡片內容的功能。
